...
/* Interface for task groups that contain multiple tasks */
interface Group {
  id: string;
  name: string;
  tasks: Task[];
...
/* Props interface for the Workspace component */
...
const Workspace = ({ isOpen, onClose, card }: WorkspaceProps) => {
  /* State management for groups, editing, and input handling */
  const [groups, setGroups] = useState<Group[]>([]);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [newName, setNewName] = useState('');
  const inputRef = useRef<HTMLInputElement>(null);

  /* Toggles completion status of a task within a group */
  ...
  /* Adds a new task to a specific group */
  ...
  /* Removes a group and all its tasks */
  const removeGroup = (groupId: string) => {
    setGroups(groups.filter(group => group.id !== groupId));
  };

  /* Removes a specific task from a group */
  ...
  /* Initiates editing mode for a group or task */
  const startEdit = (id: string, currentName: string) => {
    setEditingId(id);
    setNewName(currentName);
  };

  /* Handles input changes when editing names */
  const handleNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setNewName(e.target.value);
  };

  /* Saves current item and creates a new one */
      ...
      if (group) {
        ...
        return updatedGroups.map(g => 
          g.id === group.id
            ? {
                ...g,
                tasks: [...g.tasks, { id: newId, text: '', completed: false }]
              }
            : g
        );
      }
  ...
  /* Handles keyboard events for saving and creating tasks */
  ...
  /* Focuses input when editing starts */
  useEffect(() => {
  ...
  const completedTasks = groups.reduce((acc, group) => 
    acc + group.tasks.filter(t => t.completed).length, 0);
  const completionPercentage = totalTasks ? Math.round((completedTasks / totalTasks) * 100) : 0;

  return (
    <div 
      className={`absolute inset-0 bg-base-200 rounded-xl transform transition-transform duration-300 ease-in-out ${
